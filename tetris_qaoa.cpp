...
void print(const vector<vector<int>> &edges) {
    for (auto edge : edges) {
        for (auto node : edge) {
            printf("%d ", node);
        }
...
bool remove_edge(vector<vector<int>> &edges, int u, int v) {
...
public:
    ...
    Gate(const char *init_name, int init_q0, int init_q1) : name(init_name), qubits({init_q0, init_q1}) {}
};

void append_gate_to_circuit(Gate gate, vector<Gate> &circuit) {
    string target_name;
    if (gate.name == "SWAP") {
        target_name = "RZZ";
    ...
    for (auto it = circuit.rbegin(); it != circuit.rend(); ++it) {
        if (((*it).name == target_name) && ((*it).qubits[0] == gate.qubits[0]) && ((*it).qubits[1] == gate.qubits[1])) {
            (*it).name = "dZZ";
            // cout << "Successfully bridged! " << endl;
            // cout << (*it).qubits[0] << " " << (*it).qubits[1] << endl;
            // cout << gate.qubits[0] << " " << gate.qubits[1] << endl;
            return;
        }
        if (((*it).qubits[0] == gate.qubits[1]) || ((*it).qubits[1] == gate.qubits[0]))
            break;
    }

    circuit.emplace_back(gate);
}

vector<Gate> get_circuit(int n, vector<int> pi, vector<vector<int>> edges, int cycle=INT_MAX) {    
    int maxCycle = n % 2 == 0 ? 2 * n - 2 : 2 * n - 4;
    int curCycle = 0;
    int num_cnots = 0;
    vector<Gate> circuit;
    for (int t = 0; t < maxCycle; t++) {
        if (t % 4 == 0) {
            for (int p = 0; p < n - 1; p += 2) {
                if (remove_edge(edges, pi[p], pi[p + 1])) {
                    append_gate_to_circuit(Gate("RZZ", p, p + 1), circuit);
                }
            }
            curCycle++;
        } else if (t % 4 == 1) {
            ...
            for (int p = 1; p < n - 1; p += 2) {
                swap(pi[p], pi[p + 1]);
                append_gate_to_circuit(Gate("SWAP", p, p + 1), circuit);
            }
        } else if (t % 4 == 3) {
            for (int p = 0; p < n - 1; p += 2) {
                swap(pi[p], pi[p + 1]);
        ...
        if (edges.size() == 0) return circuit;
    }
    if (n % 2 == 1) {
        for (int p = 1; p < n - 1; p += 2) {
            swap(pi[p], pi[p + 1]);
...
int main(int argc, char *argv[]) {
    ...
    printf("Qubit mapping search done.\n");
#endif
    int num_nodes = queryGraph.num_nodes();
    auto M = result[0];
    vector<int> pi;
    for (int i = 0; i < num_nodes; i++) pi.emplace_back(i);

    for (int i = 0; i < num_nodes; i++) {
        printf("Map node %d -> qubit %d\n", i, M.getValueByKey(i));
        pi[M.getValueByKey(i)] = i;
    }

    vector<Gate> circuit = get_circuit(num_nodes, pi, queryGraph.get_edges());
    int num_cnots = 0;
    for (auto gate : circuit) {
        cout << gate.name << " " << gate.qubits[0] << " " << gate.qubits[1] << endl;
        if (gate.name == "RZZ")
            num_cnots += 2;
        else if (gate.name == "SWAP" || gate.name == "dZZ")
            num_cnots += 3;
    }
    printf("CNOT cost: %d\n", num_cnots);
    return 0;
